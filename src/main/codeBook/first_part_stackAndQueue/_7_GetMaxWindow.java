 ...
 * {4,[3,5,4],3,3,6,7}  最大值=5
 * {4,3,[5,4,3],3,6,7}  最大值=5 以此类推
 * <p> qmax双向队列特性：1）存放的是数组索引，2）数组最大值放前面（小于的值直接poll），3）数组索引从小到大
 * 解法：维护一个qmax的双向队列，用来表示当前窗口的最大值对应的数组下标索引，最大值放在队首，qmax内严格遵守大值的索引在前面，并且索引是有顺序的（因为是从前往后滑动，并且会顺序删除元素，这个是为了解释后面的w-i==peekFirst就表示peekFirst已过期）！！。
...
public class _7_GetMaxWindow {
    ...
    public static int[] getMaxWindow(int[] arr, int w) {
        ...
        for (int i = 0; i < arr.length; i++) {  // 一步一步滑动arr
            ...
            if (qmax.peekFirst() == i - w) {  // i-w就是上一个窗口开头索引，其中i是会改变的，所以是会一直向前滑的。peekFirst是索引最小的且值最大的，
